...
export function usnCalendar(
  ...
  if (opts.vat) {
    ...
    for (const { q, y, m } of vatQuarters) {
      events.push(
        makeEvent(makeDate(y, m, 25), 'report', `Декларация по НДС за ${q} квартал`, null, 'до 25 числа, через оператора ЭДО'),
      );
      // НДС уплачивается равными долями (1/3) не позднее 28 числа КАЖДОГО из трёх
      // месяцев, следующих за кварталом (п. 1 ст. 174 НК РФ).
      for (let i = 0; i < 3; i++) {
        events.push(
          makeEvent(
            makeDate(y, m + i, 28),
            'payment',
            `Уплата НДС за ${q} квартал — ${i + 1}/3`,
            null,
            'равными долями до 28 числа трёх следующих месяцев'
          )
        );
      }
    }
