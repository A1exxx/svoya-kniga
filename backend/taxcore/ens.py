...
Сроки (с переносом с выходных; госпраздники проверять отдельно):
...
Суммы авансов/уведомлений подставляются ТОЛЬКО из полного поквартального расчёта
(usn с 4 периодами). Для годового расчёта (usn_quick, 1 период) квартальные суммы
оставляем пустыми (None), чтобы НЕ задвоить годовой налог в квартальные слоты.
"""
...
def usn_calendar(
    ...
    Квартальные авансы заполняются суммами ТОЛЬКО при полном поквартальном расчёте
    (ровно 4 периода); иначе остаются None — это защищает от двойного счёта годового
    налога, когда передан результат usn_quick (1 период «год»).
    """
    has_quarterly = usn is not None and len(usn.periods) == 4

    def adv(i: int) -> Decimal | None:
        # i: 0 — Q1, 1 — полугодие, 2 — 9 месяцев. Последний период (год) сюда не попадает.
        if has_quarterly and i < 3:
    ...
    if usn is not None and usn.year_overpayment and usn.year_overpayment > 0:
        events.append(
            CalendarEvent(
                date(tax_year + 1, 4, 28), "notification",
                "Переплата по УСН за год — к зачёту/возврату (ст. 78 НК РФ)",
                usn.year_overpayment,
            )
        )

    # перенос с выходного на рабочий день (госпраздники не учитываются — сверять вручную)
    for e in events:
